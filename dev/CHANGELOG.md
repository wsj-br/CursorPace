...
- **Added**: settings - **Settings** → **Startup & Display** → **Every sample up to** chooses **2D**, **4D**, **7D**, or **14D**. The default is **14D**. **2W** plots every sample at that default. **1M** keeps the last sample of each day.
...
- **Added**: chart - range buttons **1D**, **2D**, **7D**, **1W**, **2W**, and **1M**. **1M** is the displayed cycle. Shorter ranges end at the current time on the live cycle and at renewal on an earlier cycle, and the start is capped at cycle start. At the default **14D** cutoff, **1D** through **2W** plot every sample; **1M** keeps the last sample of each local day.
...
- **Fixed**: tray - wire Open/Quit commands onto the existing `App.axaml` `NativeMenu` instead of replacing `TrayIcon.Menu`, which crashed on macOS with `The menu being updated does not match`.
...
- **Changed**: install - Avalonia, `Avalonia.Desktop`, `Avalonia.Themes.Fluent`, and `Avalonia.Fonts.Inter` 12.1.1 to 12.1.2. `Avalonia.Controls.WebView` stays at 12.1.0 (newest published matching that line).
