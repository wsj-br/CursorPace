...
## Project
Desktop app that tracks Cursor quota allowances across a billing cycle. Users sign in with a Cursor account; an embedded native WebView session fetches `https://cursor.com/api/usage-summary` (dashboard endpoint, not a documented public personal-plan API). The billing cycle and usage samples come from Cursor. The chart shows two independent percentages: Cursor models vs other models.
...
- Startup: `IStartupRegistration` via `StartupRegistration.Create()` (Windows Run key, macOS `SMAppService.mainApp` / attributed Launch Agent fallback, Linux XDG autostart). On macOS 13+ a properly signed `.app` registers itself with `SMAppService.mainApp`, which is the **Open at Login** path and supplies the app name and icon. Unsigned or older systems fall back to a Launch Agent that runs `/usr/bin/open -a "<bundle>"` (plus `-g --args --background` when **Start in notification tray** is on), with `AssociatedBundleIdentifiers` set to `com.cursorpace.app`; never exec `Contents/MacOS/CursorPace`. launchd exec of the inner binary is a background item: the process is not a GUI app, the tray icon is missing, and Accessory cannot hide the Dock. Unpackaged `dotnet run` still registers the executable. Linux AppImage autostart must use the `APPIMAGE` env var, not `Environment.ProcessPath` (that is the `/tmp/.mount_*` FUSE path and does not survive reboot). Linux autostart `Exec` must set `APPIMAGELAUNCHER_DISABLE=1` so AppImageLauncher does not start a second process. `LinuxStartupRegistration.RefreshCurrentAutostart()` runs at process start, before the single-instance lock: if `cursor-pace.desktop` already exists and this process is an AppImage, rewrite `TryExec`/`Exec` to that image and keep `--background`. A second-instance upgrade exit must not leave login pointed at the old file. Do not retarget autostart from `dotnet` / unpackaged runs on that early path.
- Linux fonts: `Startup.Main` calls `LinuxFontSetup.Apply()` before `Program` (and Avalonia) loads. It sets `FC_FONTATIONS=0` via `setenv` and writes `%LocalAppData%/CursorPace/fonts.conf` that includes the previous `FONTCONFIG_FILE` (or `/etc/fonts/fonts.conf`) and rejects `.woff` / `.woff2` (Zorin OpenDyslexic WOFF wins `$Default` and hangs or crashes Skia). `AppFonts` sets `$Default` to embedded Inter (`fonts:Inter#Inter`), not a fontconfig family name. `scripts/dev.sh` also exports `FC_FONTATIONS=0` because Cursor's terminal inherits `FC_FONTATIONS=1`.
...
- macOS Dock identity: `MacDesktopIntegration.EnsureDockIcon()` loads `Assets/cursor_pace.png` from the output directory and sets `NSApplication.applicationIconImage`. `Window.Icon` and `TrayIcon` do not change the Dock when the process is not inside `CursorPace.app`. The release bundle still uses `CFBundleIconFile` from `scripts/build-appbundle.sh`. `MacDesktopIntegration.ApplyDockVisibility` switches `NSApplication` to `Accessory` while the main window is hidden or minimized and back to `Regular` before it is shown. Do not set `LSUIElement` or replace Avalonia's AppDelegate. Restore from the tray, `--show`, or a second interactive launch; a hidden Dock icon cannot receive a Dock click.
- Tray: `ITrayService` / `TrayService` (Avalonia `TrayIcon`). Lives for the whole process.
- UI state: `MainViewModel` and `UsageChartViewModel`. Views subscribe to VM events; they do not own cycle math. The main window shows the usage chart or Settings. Settings is a page in the main window (`Back` and a `Settings` heading below the title bar), not a second window. Top tabs group the page: Startup & Display, Cursor account, Sync Server, Export & Backup, and About. The last selected tab is stored as `settingsTab` and restored the next time Settings opens, including in a later session. The Sync Server tab links to `AppInfo.SyncServerRepositoryUrl` (`https://github.com/wsj-br/CursorPace-SyncServer`): install instructions when URL or API token is missing, otherwise the repository link. The About tab shows version, UTC build date and time (`dd-MMM-yyyy HH:mm:ss UTC`), copyright, MIT license, and the GitHub repository link from `AppInfo`. Summary cards show cycle start above next renewal, the two run-out times, and the newest in-cycle sample with Cursor %, Other %, and linear Expected usage %.
...
Chart and interpolation use elapsed seconds from `CycleStart` (`CycleCalculator.AxisSeconds` / `CycleSeconds`, ticks over `TicksPerSecond`). Ranges are `1D`, `2D`, `7D`, `1W`, `2W`, and `1M`. `1D` is 24 elapsed hours and `2D` is 48. `7D` and `1W` are both seven elapsed days, and `2W` is 14. **Settings** → **Startup & Display** → **Every sample up to** is `2D`, `4D`, `7D`, or `14D`, stored as `rawSampleMaxDays` (default `14`). A preset or zoom whose nominal length is within that cutoff plots every in-cycle sample. `1M` always keeps the last sample of each local date. A capped live window still uses its preset's nominal length. `1M` is the whole displayed cycle. On the live cycle a short range ends at the clock, clamped to `NextRenewal`; on an archived cycle it ends at that cycle's `NextRenewal`. The start is always clamped to `CycleStart`. A primary-button drag across the plot zooms to that cycle-relative interval. The drag is clamped to the current plot; the committed interval then stays fixed inside the cycle until the user chooses a range button or right-clicks. Right-click selects `1M`. Choosing a range button, including the button that is already selected, clears the zoom and shows that preset. No range button is highlighted while a zoom is active. A zoom up to the configured cutoff plots every sample and uses hour slots; a longer zoom keeps the last sample of each local date. Preset buttons keep their own sampling, so a short live `7D` still uses the seven-day cutoff, not the visible-window length. The zoom is transient and clears when the displayed cycle bounds change. The visible Y axis is the min and max of the plotted values, rounded outward to 10% steps, with a non-zero span. The dashed **Expected usage** series remains a straight line from `(0, 0%)` to `(CycleSeconds, 100%)`, clipped to the viewport. When the plot uses every sample, each measured Cursor and Other Models point is a small filled circle in that series color. Clipped boundary points, the expected line, and the estimated lines are not marked. Longer ranges have no sample circles. The last measured point inside the viewport is labelled to its right as `xx.x%` in that series color. A dotted vertical guide at that X rises to `LinearExpectedPercent`. Pointer hover interpolates the displayed Cursor, Other, and Expected polylines; an observed value outside its polyline is blank. While a custom zoom is active, the readout also shows each metric at the zoom start and the change from that start to the pointer. Preset ranges hide those rows. A delta is blank when either observed value is blank. CSV expected values still use `ExpectedPercentAt`.
...
Daily burn, projected percents, and run-out are a separate Theil-Sen series on last-of-day sample second-offsets (plus a 0% origin when the cycle-start date has no in-cycle sample) and must not replace `ExpectedPercent`. `EstimateDailyUsage` is `ratePerSecond * 86400`. Solid estimated lines are two points: last sample to `NextRenewal` via `ProjectedPercentAt`. `EstimateRunOutInstant` is the source; `EstimateRunOutDayNumber` maps it onto a cycle day or returns null if the instant is outside 1..D. CSV and tray today percents use the last in-cycle sample of that local date when one exists; otherwise `ExpectedPercent` at that day's midnight. Estimated percents are calculated for days after the last sample.

Chart slots for multi-day ranges are local midnights inside the visible window. On a timed cycle slot 0 of the full cycle is the truncated `[CycleStart, first midnight)` and carries `IsLeadingPartial`; it gets no label because it is too narrow and its date repeats on the next slot. Every other day slot is labelled with `Date.Day`. `1D` and `2D` use hour slots and label the local time. Thin day labels by slot index, never by the day value, or spacing breaks at a month boundary. Day-range gridlines sit at slot starts except the plot's left edge. An intraday axis picks a 1, 2, 3, 4, 6, 8, 12, or 24 hour interval so the time labels do not overlap, and draws grid lines only on that interval.
...
A signed-in snapshot with a new cycle start date archives the previous `ActiveCycle` into `cycleHistory` and replaces `ActiveCycle`. `UsageSampleAppender` keeps previous samples and always appends the first sample of the new cycle; the 30-second min-gap applies only within the same cycle. The chart, info cards, and **Export Cycle CSV** follow the displayed cycle. Tray tooltip, today percents, and `CheckForNewDay` use the live `ActiveCycle`. Launch and backup restore always show the live cycle. Previous/Next on the shared month heading pages stored cycles (commands on `MainViewModel`).

If you change `CycleCalculator`, `QuotaCycle`, or `JsonPlanStore` serialization, update and run `Tests/CycleCalculatorTests.cs`. Sample-driven expected/estimate cases live in `Tests/SampleEstimationTests.cs`. If you change chart range, zoom, hover interpolation, or X/Y mapping, update `Tests/UsageChartSeriesBuilderTests.cs`, `Tests/UsageChartRangeTests.cs`, `Tests/UsageChartViewModelTests.cs`, and `Tests/MainViewModelTests.cs`. If you change launch/interval skip rules, update `Tests/SyncScheduleTests.cs`. If you change `--background` / `--show` / **Start in notification tray** launch hiding, update `Tests/LaunchModeTests.cs`. If you change Linux autostart executable resolution, update `Tests/LinuxStartupRegistrationTests.cs`. If you change Linux fontconfig startup (`LinuxFontSetup` / `AppFonts`), update `Tests/LinuxFontSetupTests.cs`. If you change Linux taskbar `.desktop` / `StartupWMClass` identity, update `Tests/LinuxDesktopIntegrationTests.cs`. If you change macOS login-item resolution or Launch Agent `open` arguments, update `Tests/MacStartupRegistrationTests.cs`. If you change the macOS Dock icon path, application, or hidden-window activation policy, update `Tests/MacDesktopIntegrationTests.cs`. If you change Settings About / `AppInfo`, update `Tests/AppInfoTests.cs`. If you change single-instance acquire or the Unix lock/socket, update `Tests/SingleInstanceTests.cs`. If you change window placement clamp or persisted size/maximized fields, update `Tests/WindowPlacementTests.cs` and `Tests/MainViewModelTests.cs`. If you change cycle history, sample rollover, or Previous/Next cycle navigation, update `Tests/UsageSampleAppenderTests.cs`, `Tests/JsonPlanStoreTests.cs`, `Tests/CycleHistoryTests.cs`, and `Tests/MainViewModelTests.cs`. If you change undecorated resize math, update `Tests/WindowResizeTests.cs`. If you change the WebView host off-screen size, slot math, or when `NativeWebView` is attached, update `Tests/WebViewHostLayoutTests.cs`. If you change remote-sync merge, HTTP contract, or Settings sync-server fields, update `Tests/RemoteSyncMergeTests.cs`, `Tests/RemoteSyncServiceTests.cs`, `Tests/JsonPlanStoreTests.cs`, and `Tests/MainViewModelTests.cs`.
...
`Export Usage` is visible when connected. There is no calendar view, **Reset**, or **Change renewal day**.
